...
<div class="container my-4">
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            ...
            @if($product->image_url)
                <form method="POST" action="{{ route('products.removeImage', $product) }}" style="display:inline;">
                    @csrf
                    <button type="submit" class="btn btn-danger btn-sm mt-2" onclick="return confirm('Remove product image?')">Remove Image</button>
                </form>
            @endif
        </div>
    </div>
